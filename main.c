...
int main() {
    ...
    // вивід прохання ввести варіант
    printf("Варіант (1-25): ");
    scanf("%d", &variant);


    // перевірка коректності варіанту
    ...
    for (unsigned int i = 1; i <= N; i++) {
        ...
        if (x > X2) x = X2;
        double fx = f(x, variant); //викликається наша функція для обрахунку f(x)

        printf("+-----------+-----------+-----------+\n");
        printf("|%11u|%11.2f|%11.2f|\n", i, x, fx);

        // перевірка на заповнення сторінки
